...
class Attention(Layer):
    def __init__(self, **kwargs):
        ...
        self.init = tensorflow.keras.initializers.get('glorot_uniform')
...
class Diagonal(Layer):
    ...
    def call(self, x, mask=None):
        ...
        mult = K.reshape(mult, (-1, tf.cast(self.n_inputs_per_node, tf.int32)))
...
def f1(y_true, y_pred):
    ...
    # main_pred = y_pred[0]  # 形状为 (None, 1)
    # features_pre = y_pred[1]  # 形状为 (None, 26, 8)
    # features_true = y_pred[2]  # 形状为 (None, 26, 8)
    def recall(y_true, main_pred):
        """Recall metric.
        ...
        """
        true_positives = K.sum(K.round(K.clip(y_true * main_pred, 0, 1)))
        possible_positives = K.sum(K.round(K.clip(y_true, 0, 1)))
        recall = true_positives / (possible_positives + K.epsilon())
        return recall

    def precision(y_true, main_pred):
        """Precision metric.
        ...
        """
        true_positives = K.sum(K.round(K.clip(y_true * main_pred, 0, 1)))
        predicted_positives = K.sum(K.round(K.clip(main_pred, 0, 1)))
        precision = true_positives / (predicted_positives + K.epsilon())
        return precision

    precision = precision(y_true, main_pred)
    recall = recall(y_true, main_pred)
