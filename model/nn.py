...
# from data.data_access_test import x_train
...
class Model(BaseEstimator):
    ...
    def fit(self, X_train, y_train, X_val=None, y_val=None):
        ...
        if self.class_weight == 'auto':
            ...
            # class_weights = class_weight.compute_class_weight('balanced', classes=classes, y=y_train.to_numpy())
        ...
        if not X_val is None:
            X_val_tensors = [tf.convert_to_tensor(x, dtype=tf.float32) for x in X_val]
            # y_val_tensors = [tf.convert_to_tensor(y_val, dtype=tf.float32)]
            validation_data_tensors = [X_val_tensors, y_val]
            # validation_data = [X_val, y_val]
        else:
            validation_data = []
        # 输出层名称
        output_layer_names = ['o1', 'o2', 'o3', 'o4', 'o5', 'o6']

        # 构建 class_weights 字典
        # class_weights = {name: class_weights for name in output_layer_names}

        X_train_tensors = [tf.convert_to_tensor(x, dtype=tf.float32) for x in X_train_list]

        # Custom callback to record training metrics (acc, auc, f1, aupr, etc.)
        class TrainingMetricsHistory(tf.keras.callbacks.Callback):
            def __init__(self, X_train, y_train, evaluate_fn):
                super().__init__()
            ...
            def on_epoch_end(self, epoch, logs=None):
                ...
                if isinstance(self.X_train, list):
                    y_pred_score = self.model.predict(self.X_train)
                    if isinstance(y_pred_score, list):  # Multi-output model
                        y_pred_score = y_pred_score[0]  # Take first output
                    y_pred = np.round(y_pred_score)
                else:
                    y_pred_score = self.model.predict(self.X_train)
                    y_pred = np.round(y_pred_score)

                # Flatten y_train if it's a list (multi-output)
                y_true = self.y_train[0] if isinstance(self.y_train, list) else self.y_train
                y_true = np.array(y_true).ravel()  # Ensure 1D array

                # Compute metrics
                metrics = self.evaluate_fn(y_true, y_pred, y_pred_score)
        ...
        # Define evaluate function (same as your `evalualte()` function)
        def evaluate(y_test, y_pred, y_pred_score):

            y_pred_score = y_pred_score[:, 0]
            y_pred =y_pred[:, 0]
            # # Ensure y_pred is class labels, not probabilities
            # if len(y_pred.shape) > 1 and y_pred.shape[1] > 1:
            #     y_pred = np.argmax(y_pred, axis=1)
            #
            # # Ensure y_test is class labels (not one-hot)
            ...
            # logging.info(
            #     f"--accuracy: {accuracy:.2f} precision: {precision:.2f} auc: {auc:.2f} f1: {f1:.2f} aupr {aupr:.2f}"
            # )
            return {
                'accuracy': accuracy,
                'precision': precision,
        ...
        # Add custom callback
        training_metrics_callback = TrainingMetricsHistory(X_train_tensors, y_train, evaluate)
        callbacks.append(training_metrics_callback)
        # # print(type(y_train))
        # y_train = tf.convert_to_tensor(y_train[0], dtype=tf.float32)
        # print(y_train)
        history = self.model.fit(X_train_tensors, y_train, validation_data=validation_data_tensors, epochs=self.nb_epoch,
                                 batch_size=self.batch_size,
                                 verbose=self.verbose, callbacks=callbacks,
                                 shuffle=self.shuffle)
        #
        # history = self.model.fit(X_train_tensors,  y_train,
        ...
        saving history
        '''
        # Merge training metrics into history
        for key in training_metrics_callback.train_metrics_history:
            history.history[f'train_{key}'] = training_metrics_callback.train_metrics_history[key]
        ...
        # if not X_val is None:
        pred_validate_score = self.get_prediction_score(X_train_tensors)
        if self.n_outputs > 1:
            y_train = y_train[0]
        ...
        #     self.coef_ = self.get_coef_importance(X_train_tensors, y_train, target=-1,
