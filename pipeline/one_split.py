...
sys.path.append('D:\studyflie\post-code\p-net1\pnet_prostate_paper')
...
import shap, lime.lime_tabular, lime.lime_text

def preprocess_sentences_for_bert(sentences, max_length=512):
    """
    将输入的句子列表预处理为BERT模型所需的输入格式。

    参数:
        tokenizer_path (str): BERT分词器的预训练模型路径。
        sentences (list of str): 输入的句子列表。
        max_length (int): 每个句子的最大长度（默认为512）。

    返回:
        list of dict: 每个句子对应的BERT输入字典，包含 'input_ids' 和 'attention_mask'。
    """
    # 加载BERT分词器
    ...
    tokens = tokenizer(
            ...
            return_tensors='tf',  # 返回普通的Python列表
            padding='max_length',  # 或 'longest' 或 'do_not_pad'
            truncation=True,
    )



    input_ids_array = np.array(tokens['input_ids'], dtype=np.int32)
    attention_mask_array = np.array(tokens['attention_mask'] , dtype=np.int32)
...
class OneSplitPipeline:
    ...
    def run(self):
        ...
        for data_params in self.data_params:
            ...
            x_validate_1, x_validate_2, x_validate_3,x_validate_4,x_validate_5 = separate_features(x_validate_, cols,file1,file2,file3)
            x_test_1, x_test_2, x_test_3,x_test_4,x_test_5 = separate_features(x_test_, cols,file1,file2,file3)
            x_train = x_train_1
            x_validate_ =x_validate_1
            x_test_ =x_test_1
            ...
            if self.eval_dataset == 'validation':
                ...
                x2_t = np.concatenate((x_test_2, x_validate_2))
                x3_t = np.concatenate((x_test_3, x_validate_3))
                x4_t = np.concatenate((x_test_4, x_validate_4))
                x5_t = np.concatenate((x_test_5, x_validate_5))
                info_t = info_test_.append(info_validate_)
                sentences  = [item for sublist in x4_t for item in sublist]
                # sentences = x4_t.tolist()
                bert_input_t,bert_mask_t = preprocess_sentences_for_bert(sentences)
            ...
            for m in self.model_params:
                ...
                #
                # print(f)
                x_train_lsit = [x_train,x_train_2,x_train_3,bert_input,bert_mask,x_train_5]
                sentences = x_validate_4['PET/CT'].tolist()
                bert_input_v,bert_mask_v = preprocess_sentences_for_bert(sentences)
                x_validate_list = [x_validate_1,x_validate_2,x_validate_3,bert_input_v,bert_mask_v,x_validate_5]
                if model_params_['type'] == 'nn' and not self.eval_dataset == 'validation':

                    model = model.fit(x_train_lsit, y_train, x_validate_list, y_validate_)
                else:
                ...
                x_test_list = [x_test,x2_t,x3_t,bert_input_t,bert_mask_t,x5_t]
                X_test_tensors = [tf.convert_to_tensor(x, dtype=tf.float32) for x in x_test_list]
                y_pred_test, y_pred_test_scores = self.predict(model, X_test_tensors, y_t)
            ...
            # 保存训练分数到CSV
        # if train_score_df:
        #     train_scores_df = pd.DataFrame(train_score_df)
        #     train_scores_df.to_csv(join(self.directory, model_name + 'train_scores.csv'), index=False)
