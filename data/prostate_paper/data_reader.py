...
def load_data(filename, selected_genes=None):
    ...
    if filename in cached_data:
        ...
        chunks = []
        for chunk in pd.read_csv(filename, chunksize=chunk_size, index_col=0, encoding='gbk'):
            # 如果需要，可以在这里对每批数据进行预处理
            if selected_genes is not None:
                chunk = chunk[selected_genes]
            chunks.append(chunk)
    ...
    # else:
    #     chunks = []
    #     for chunk in pd.read_csv(filename, chunksize=chunk_size, index_col=0):
    #         # 如果需要，可以在这里对每批数据进行预处理
    #         if selected_genes is not None:
    #             chunk = chunk[selected_genes]
    #         chunks.append(chunk)
...
class ProstateDataPaper():
    ...
    def __init__(self, data_type='mut', account_for_data_type=None, cnv_levels=5,
        ...
        if type(data_type) == list:
            ...
            # x = pd.DataFrame(x, columns=cols)  x = pd.DataFrame(x, columns=sorted(cols))
